...
public class LocalityAndStreetsDB {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        System.out.println("Текущая информация о населенных пунктах и улицах: ");
        ...
        switch (action) {
            case 1 -> {
                ...
                switch (input.nextInt()) {
                    case 1 -> {
                        switch (action) {
                            case 1 -> TableAction.insertLocality(preparedStatement, connection);
                            case 2 -> {
                                PrintResult.printTableLocality(statement);
                                TableAction.modifyLocality(statement, connection);
                            }
                            case 3 -> {
                                PrintResult.printTableLocality(statement);
                                TableAction.deleteEntry("id_locality", "locality", statement, connection);
                            }
                    ...
                    case 2 -> {
                        switch (action) {
                            case 1 -> {
                                PrintResult.printTableLocality(statement);
                                TableAction.insertStreet(preparedStatement, connection);
                            }
                            case 2 -> {
                                PrintResult.printTableStreets(statement);
                                TableAction.modifyStreet(statement, connection);
                            }
                            case 3 -> {
                                PrintResult.printTableStreets(statement);
                                TableAction.deleteEntry("id_street", "streets", statement, connection);
                            }
                            default -> System.out.println("Неревно введён номер действия.");
                        }
                    }
            ...
            case 2 -> {
                ...
                PrintResult.printLocalityInformation(input.next(), preparedStatement, connection);
            }
            case 3 -> {
                System.out.println("""
                ...
                switch (input.nextInt()) {
                    case 1 -> {
                        System.out.println("Введите название населенного пункта: ");
                        SearchInfo.searchAllStreetsLocality(input.next(), preparedStatement, connection);
                    }
                    case 2 -> {
                        System.out.println("Введите название: ");
                        SearchInfo.searchAllLocalityWithStreet(input.next(), preparedStatement, connection);
                    }
            ...
            case 4 -> {
                ...
                switch (action) {
                    case 1 -> {
                        ...
                        SearchInfo.searchByName(input.next(), "name_locality", "locality", statement);
                    }
                    case 2 -> {
                        System.out.println("Введите количество жителей(в тыс.): ");
                        SearchInfo.searchBySquareOrPeople(input.nextFloat(), "people", statement);
                    }
                    case 3 -> {
                        System.out.println("Введите площадь: ");
                        SearchInfo.searchBySquareOrPeople(input.nextFloat(), "square", statement);
                    }
                    case 4 -> {
                        System.out.println("Введите дату основания(ГГГГ-ММ-ДД): ");
                        SearchInfo.searchByDate(input.next(), preparedStatement, connection);
                    }
                    case 5 -> {
                        System.out.println("Введите полное или частичное название улицы: ");
                        SearchInfo.searchByName(input.next(), "name_street", "streets", statement);
                    }
                    case 6 -> {
                        System.out.println("Введите тип улицы: ");
                        SearchInfo.searchByTypeStreet(input.next(), preparedStatement, connection);
                    }
                    default -> System.out.println("Неревно введён номер действия.");
                }
            }
